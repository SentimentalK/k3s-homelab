...
## Step 5: Apply Ingress & Apps Bootstrapping
Expose the ArgoCD server web UI and bootstrap all other cluster tools and applications:
```bash
# 1. Deploy Ingress routing to the HTTP port of argocd-server
sudo kubectl apply -f bootstrap/argocd-ingress.yaml

# 2. Apply the app-of-apps bootstrap configuration
sudo kubectl apply -f bootstrap/argocd-apps.yaml
```

---

## Step 6: Verify and Fetch Sealed Secrets Certificate
Wait for the apps to synchronize and fetch the sealed secrets public certificate for encryption:
```bash
# 1. Verify applications sync status (should transition to Synced)
sudo kubectl get applications -n argocd

# 2. Verify sealed-secrets pod is running in kube-system
sudo kubectl get pods -n kube-system | grep sealed

# 3. Fetch the public certificate
